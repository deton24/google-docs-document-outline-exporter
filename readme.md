...
<br>
It's messy - it was vibecoded with free Claude Sonnet 4.5 without deep thinking enabled (it couldn't finish tasks with it - the same for Sonnet 4.6 without deep thinking). The previous version of the script (called old) only copies all the headings similarly looking like in the opened DOCX downloaded from GDoc (so with all present headings in the documents). <br> Also, I tried exporting the list of headings present in only document outline using "Document Outline by Alice Keeler" from, Google Workspace Marketplace, but it created pretty much messsed up outline - some entries were unnecessary, some were lacking (also it didn't work at first, and I think I needed to refresh the GDoc once or twice to make it work - then it started writing the output as new GDoc files on my GDrive.
